...
const IGNORE = new Set(["", "lab", "个人网站", "杂项", "学校网站"]);
...
const main = async (navigations: BookmarkItem) => {
  try {
    ...
    await writeFile(
      "navigator.ts",
      `export const navigator = ${JSON.stringify(navigations)}`,
      { signal },
    );
...
const navigations = purifying(
  children.find((child) => child.title === "toolbar"),
);
